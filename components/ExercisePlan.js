...
import { workoutPlans } from './exercisePlans';

const ExercisePlans = () => {
  const [selectedLevel, setSelectedLevel] = useState('Beginner');

  const filteredPlans = workoutPlans.filter(plan => plan.level === selectedLevel);

  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold mb-4">Exercise Plans for Diabetics</h2>
      <div className="mb-4">
        <label htmlFor="level" className="mr-2">Choose your level:</label>
        <select
          id="level"
          value={selectedLevel}
              ...
              <ul className="list-disc list-inside">
                {plan.workouts.map((workout, index) => (
                  <li key={index} className="mb-2">
                    <strong>{workout.name}:</strong> {workout.description} ({workout.duration} mins)
                    <p className="text-sm text-green-600">{workout.benefit}</p>
                  </li>
                ))}
              </ul>
            </div>
