...
// Function to remove today's date from completed dates in Firestore
export async function removeCompletedDate() {
  try {
    const user = auth.currentUser;
    if (!user) {
      console.error("User not authenticated");
    ...
    if (docSnap.exists()) {
      // Get current completedDates array and filter out today's date
      const completedDates = docSnap.data().completedDates || [];
      const updatedDates = completedDates.filter(date => date !== dateString);
      
      // Update document with the filtered dates array
      await setDoc(userCompletionsRef, {
...
// Calculate the maximum streak of consecutive dates
export async function calculateMaxStreak() {
  try {
    ...
    for (let i = 1; i < sortedDates.length; i++) {
      ...
      if (Math.round(diffDays) === 1) {
        ...
        // Same date (shouldn't happen but just in case)
        continue;
      } else {
        // Non-consecutive date, reset streak
        currentStreak = 1;
      }
    }
